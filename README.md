...
1. Determines the video's LANK (its `pub-…_VIDEO` identifier) from the page URL,
   the embedded player markup (including WOL’s `[data-json-src]`), a jw-cdn download link, the `docid`/`track`
...
`install.html` embeds both builds. To rebuild and run the DOM regression tests:
...
The build URL-encodes the bookmarklet so dragging it to the bookmarks bar preserves
newlines and special characters. Tests cover WOL markup before and after its player
loads, plus the bookmark URL generated by the installer.
